...
test("getPersonalBestStatus treats a first win as a new time and moves record", () => {
  assert.deepEqual(
...
test("getPersonalBestStatus returns both flags when both metrics improve", () => {
  assert.deepEqual(
    getPersonalBestStatus({ bestTimeLeft: 30, fewestMoves: 20 }, { moves: 15, timeLeft: 50 }),
    {
      hasNewPersonalBest: true,
      isNewBestMoves: true,
      isNewBestTime: true,
